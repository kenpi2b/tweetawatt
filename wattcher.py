...
handler = logging.FileHandler('/home/pi/wattcher/wattcher.log')
formatter = logging.Formatter('%(asctime)s : %(filename)s : %(levelname)s : %(message)s','%Y-%m-%d %H:%M:%S %a')
...
# use App Engine? or log file? To use app engine, must have LOGFILENAME declared? bug?
LOGFILENAME = "powerdatalog.csv"    # where we will store our flatfile data

LOGTODB = False						# Choose to log data to a database
LOGTOEMONCMS = True					# Log data to emoncms server
LOGTOFILE = False					# Choose to log data to a csv file
ENABLETWITTER = False				# Choose to Twitter data
...
if LOGTODB:
	...
	dbtable = ""			# name of device (no symbols except underscores)
	logger.info("Logging to %s.%s" % (database,dbtable)) 
...
vrefcalibration = [492,  # Calibration for sensor #0
                   512,  # Calibration for sensor #1 # change this one since I'm using Sensor 1
...
def TwitterIt(u, p, message):
    ...
    try:
        ...
        logger.info("%s just posted: %s" % (status.user.name, status.text))
    except UnicodeDecodeError:
        print "Your message could not be encoded.  Perhaps it contains non-ASCII characters? "
        print "Try explicitly specifying the encoding with the  it with the --encoding flag"
        logger.warning("UnicodeDecodeError : Twitter status update failed")
    except:
        print "Couldn't connect, check network, username and password!"
        logger.warning("Twitter connection failed : Check network, usename, password")
...
logfile = None # for some reason this is required for xbee comm
...
def update_graph(idleevent):
    ...
    if (((time.time() - sensorhistory.fiveminutetimer) >= 60.0) # 60.0
        and (currminute % 2 == 0)): # was %5 == 0
        ...
        if LOGTODB:
			try:
				...
				print "Error %d: %s" %(e.args[0],e.arges[1])
				logger.error("Error %d: %s" % (e.args[0],e.args[1]) )
			else:
				try:
					# prepare a cursur object using cursur() method
					dbcursor = db.cursor()
				#except MySQLdb.Error, e:
				#	print "Error %d: %s" % (e.args[0],e.args[1])
				...
				#try:	
					...
					# --> all data must be type string for MySQL query
					data = {
						'table': str(dbtable),
						'amps': str(avgamp),
						'watts': str(avgwattsused),
						'date': time.strftime("%Y-%m-%d"),
						'time': time.strftime("%H:%M:%S"),
						'sensornum': str(sensorhistory.sensornum)
					}
					sql = """INSERT INTO """ + dbtable + """ (amps,watts,date,time,sensornum) VALUES (%(amps)s,%(watts)s,%(date)s,%(time)s,%(sensornum)s)"""
					# execute SQL command
					dbcursor.execute(sql, data)
					# commit changes to the DB
					db.commit()
					# print message
					print "Data sent to database: %s.%s" % (database,dbtable)
				except MySQLdb.Error, e:
					# Rollback in case an error happens
					print "Error %d: %s" % (e.args[0],e.args[1])
					logger.error("Database error : rollback() called")
					logger.error("Error %d: %s" % (e.args[0],e.args[1]) )
					db.rollback()
				except:
					print "Error %d: %s" % (e.args[0],e.args[1])
...
#    		if LOGTODB:
					try:
						...
						print "Database error : failed to close database\n"
						print "Error %d: %s" % (e.args[0],e.args[1])
						logger.error("Database error : failed to close database")
						logger.error("Error %d: %s" % (e.args[0],e.args[1]) )
#						db.rollback();
					except:
						print "Error %d: %s" % (e.args[0],e.args[1])
